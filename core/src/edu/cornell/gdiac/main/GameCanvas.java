 ...
 * <p>
 * This version of GameCanvas only supports both rectangular and polygonal Sprite drawing.  It also
 * supports a debug mode that draws polygonal outlines.  However, that mode must be done in a
 * separate begin/end pass.
 */
public class GameCanvas {
   ...
   * <p>
   * Width, height, and fullscreen are taken from the LWGJApplicationConfig object used to start the
   * application.  This constructor initializes all of the necessary graphics objects.
   */
  public GameCanvas() {
   ...
   * <p>
   * This method raises an IllegalStateException if called while drawing is active (e.g. in-between
   * a begin-end pair).
   *
  ...
  public void setWidth(int width) {
    if (active != DrawPass.INACTIVE) {
      Gdx.app.error("GameCanvas", "Cannot alter property while drawing active",
          new IllegalStateException());
      return;
    }
    this.width = width;
   ...
   * <p>
   * If desktop is true, it will use the current desktop resolution for fullscreen, and not the
   * width and height set in the configuration object at the start of the application. This
   * parameter has no effect if fullscreen is false.
   * <p>
   * This method raises an IllegalStateException if called while drawing is active (e.g. in-between
   * a begin-end pair).
   *
   ...
   * <p>
   * Any texture draw subsequent to this call will use the rules of this blend state to composite
   * with other textures.  Unlike the other setters, if it is perfectly safe to use this setter
   * while  drawing is active (e.g. in-between a begin-end pair).
   *
   ...
   * <p>
   * The texture colors will be multiplied by the given color.  This will turn any white into the
   * given color.  Other colors will be similarly affected.
   * <p>
   * Unless otherwise transformed by the global transform (@see begin(Affine2)), the texture will be
   * unscaled.  The bottom left of the texture will be positioned at the given coordinates.
   *
   ...
   * @param height The texture height
   */
  public void draw(Texture image, Color tint, float ox, float oy, float x, float y, float width,
      float height) {
    if (active != DrawPass.STANDARD) {
      Gdx.app.error("GameCanvas", "Cannot draw without active begin()",
          new IllegalStateException());
   ...
   * given color.  Other colors will be similarly affected.
   * <p>
   * The transformations are BEFORE after the global transform (@see begin(Affine2)). As a result,
   * the specified texture origin will be applied to all transforms (both the local and global).
   * <p>
   * The local transformations in this method are applied in the following order: scaling, then
   * rotation, then translation (e.g. placement at (sx,sy)).
   *
   * @param image The texture to draw
   * @param tint  The color tint
   * @param ox    The x-coordinate of texture origin (in pixels)
   ...
   * <p>
   * A texture region is a single texture file that can hold one or more textures. It is used for
   * filmstrip animation.
   * <p>
   * The texture colors will be multiplied by the given color.  This will turn any white into the
   * given color.  Other colors will be similarly affected.
   ...
   * <p>
   * Unless otherwise transformed by the global transform (@see begin(Affine2)), the texture will be
   * unscaled.  The bottom left of the texture will be positioned at the given coordinates. region
   *
   * @param image  The texture to draw
   * @param tint   The color tint
   * @param x      The x-coordinate of the bottom left corner
   ...
   * @param height The texture height
   */
  public void draw(TextureRegion region, Color tint, float ox, float oy, float x, float y,
   ...
   * <p>
   * A polygon region is a texture region with attached vertices so that it draws a textured
   * polygon. The polygon vertices are relative to the texture file.
   * <p>
   * The texture colors will be multiplied by the given color.  This will turn any white into the
   * given color.  Other colors will be similarly affected.
   * <p>
   ...
   * @param height The texture height
   */
  public void draw(PolygonRegion region, Color tint, float ox, float oy, float x, float y,
      float width, float height) {
    if (active != DrawPass.STANDARD) {
      Gdx.app.error("GameCanvas", "Cannot draw without active begin()",
          new IllegalStateException());
   ...
   */
  public void drawPhysics(PolygonShape shape, Color color, float x, float y, float angle, float sx,
      float sy) {
    if (active != DrawPass.DEBUG) {
      Gdx.app.error("GameCanvas", "Cannot draw without active beginDebug()",
          new IllegalStateException());
   ...
   * Draws the outline of the given shape in the specified color
   * <p>
   * The position of the circle is ignored.  Only the radius is used. To move the circle, change the
   * x and y parameters.
   *
   * @param shape The Box2d shape
   * @param color The outline color
   ...
   * <p>
   * For reasons of convenience, we do not allow user-defined blend functions. 99% of the time, we
   * find that the following blend modes are sufficient (particularly with 2D games).
   */
