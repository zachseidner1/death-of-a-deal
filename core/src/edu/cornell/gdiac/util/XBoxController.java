 ...
 * <p>
 * This is a wrapper class, which wraps around a Controller object to provide concrete mappings for
 * the buttons, joysticks and triggers.  It is simpler than having to remember the exact device
 * numbers (even as constants) for each button. It is particularly important because different
 * operating systems have different mappings for the buttons.
 * <p>
 * Each controller must have its own instance.  The constructor automatically determines what OS
 * this controller is running on before assigning the mappings. The constructor DOES NOT verify that
 * the controller is indeed an XBox 360 controller.
 */
public class XBoxController implements Controller, ControllerListener {
   ...
   * <p>
   * Due to joystick sensitivity, we allow some minor dead zone settings. Any joystick value within
   * this distance of 0 will register as 0.
   *
   ...
   * <p>
   * This is method only returns true if the up button is pressed by itself.  If the up button is
   * combined with left or right, it will return false.  For more flexible usage of the DPad, you
   * should use the method getDPadDirection().
   * <p>
   * This method returns false if the controller is disconnected.
   *
   ...
   * <p>
   * The result will be one of the eight cardinal directions, or center if the DPad is not actively
   * pressed.
   * <p>
   ...
   * <p>
   * This ID is used to recognize this controller if more than one of the same controller models are
   * connected. Use this to map a controller to a player, but do not use it to save a button
   * mapping.
   * <p>
   * This method returns null if the controller is disconnected.
   *
   ...
   * <p>
   * Axis indices start at 0, so the maximum axis index is one under this value. On XBoxes, the high
   * end axes are the triggers.
   * <p>
   ...
   * <p>
   * Note that this is no guarantee that the connected controller itself can vibrate. Simply that
   * the system believes that it can.
   * <p>
   * This method returns false if the controller is not connected.
   *
   ...
   * <p>
   * This not be supported by some implementations. This method does nothing if the controller is
   * not connected.
   */
  @Override
   ...
   * <p>
   * This returns PLAYER_IDX_UNSET if none is set, or if the controller is disconnected.
   *
   ...
   * <p>
   * Please note that this does not always set indication lights of controllers. It is just an
   * internal representation on some platforms.  Set this to PLAYER_IDX_UNSET to unset the index.
   * <p>
   ...
   * <p>
   * The connected controller might not support all features.	 This method returns null if the
   ...
   * {@link ControllerPowerLevel#POWER_UNKNOWN} if information is not present (including when the
   ...
   * <p>
   * The listener will receive calls in case the state of the controller changes. The listener will
   ...
   * <p>
   * This method does nothing if the controller is disconnected or the given listener is not
   * registered.
   *
   ...
   * <p>
   * Low-level device polling can only ask about individual d-pad buttons. However, the primary
   * usage of a d-pad is for directional information. That is encapsulated in this enum.
   */
