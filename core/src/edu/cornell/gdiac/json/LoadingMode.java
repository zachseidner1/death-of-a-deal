 ...
 * <p>
 * You still DO NOT need to understand this class for this lab.  We will talk about this class much
 * later in the course.  This class provides a basic template for a loading screen to be used at the
 * start of the game or between levels.  Feel free to adopt this to your needs.
 * <p>
 * You will note that this mode has some textures that are not loaded by the AssetManager. You are
 * never required to load through the AssetManager.  But doing this will block the application. That
 * is why we try to have as few resources as possible for this loading screen.
 */
public class LoadingMode implements Screen, InputProcessor, ControllerListener {
  ...
  private void drawProgress(GameCanvas canvas) {
    ...
    canvas.draw(statusBkgRight, Color.WHITE,
        centerX + width / 2 - scale * statusBkgRight.getRegionWidth(), centerY,
        scale * statusBkgRight.getRegionWidth(), scale * statusBkgRight.getRegionHeight());
    canvas.draw(statusBkgMiddle, Color.WHITE,
        centerX - width / 2 + scale * statusBkgLeft.getRegionWidth(), centerY,
    ...
    if (progress > 0) {
      float span = progress * (width - scale * (statusFrgLeft.getRegionWidth()
          + statusFrgRight.getRegionWidth())) / 2.0f;
      canvas.draw(statusFrgRight, Color.WHITE,
          centerX - width / 2 + scale * statusFrgLeft.getRegionWidth() + span, centerY,
          scale * statusFrgRight.getRegionWidth(), scale * statusFrgRight.getRegionHeight());
      canvas.draw(statusFrgMiddle, Color.WHITE,
          centerX - width / 2 + scale * statusFrgLeft.getRegionWidth(), centerY,
   ...
   * <p>
   * This method checks to see if the play button is available and if the click is in the bounds of
   * the play button.  If so, it signals the that the button has been pressed and is currently down.
   ...
   * <p>
   * This method checks to see if the play button is currently pressed down. If so, it signals the
   * that the player is ready to go.
   *
   * @param screenX the x-coordinate of the mouse on the screen
   * @param screenY the y-coordinate of the mouse on the screen
   * @param pointer the button or touch finger number
   ...
   * <p>
   * The buttonCode is controller specific. This listener only supports the start button on an X-Box
   * controller.  This outcome of this method is identical to pressing (but not releasing) the play
   * button.
   *
   ...
   * <p>
   * The buttonCode is controller specific. This listener only supports the start button on an X-Box
   * controller.  This outcome of this method is identical to releasing the the play button after
   * pressing it.
   *
   * @param controller The game controller
   * @param buttonCode The button pressed
   ...
   * <p>
   * Reason may be from OS interruption to touch becoming a large surface such as the user cheek.
   * Relevant on Android and iOS only. The button parameter will be Input.Buttons.LEFT on iOS.
   *
   * @param screenX the x-coordinate of the mouse on the screen
   * @param screenY the y-coordinate of the mouse on the screen
   * @param pointer the button or touch finger number
