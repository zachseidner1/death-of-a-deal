 ...
 * <p>
 * Film strips are defined inside of a texture entry.  That texture entry is the source of the film
 * strip. Film strips are defined inside of a subobject called "sprites". They have the following
 * entries:
 * <p>
 * * "rows": The number of rows in the sprite sheet * "cols": The number of columns in the sprite
 * sheet * "size": The number of frames in the sprite sheet * "region": The subregion of the texture
 * defining the sprite sheet
 * <p>
 * The region has the same format as {@link TextureRegionParser}. The values size and region are
 * optional.
 */
public class FilmStripParser implements AssetParser<FilmStrip> {
   ...
   * <p>
   * The parser converts JSON entries into {@link FilmStripLoader.FilmStripParameters} values of the
   * same name. The keys match the field names of that object.
   * <p>
   ...
   * <p>
   * Filmstrips are defined inside of a parent texture. As not all textures have a film strip, we
   * need to scan forward to find the next film strip
   */
