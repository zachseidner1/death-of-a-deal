...
public class ParserUtils {
   ...
   * <p>
   * The texture filter is specified by a string of lowercase letters. Compound words are separated
   * by spaces. So "auto medium" is AutoMedium.
   *
   ...
   * <p>
   * A color may be (1) an array of floats 0-1, (2) a hex string, or (3) a name.  The only names
   * recognized are the constants in {@link Color}.  If the contents of json do not match any of
   * these formats, this method uses defaultColor instead.
   *
   * @param json         The JSON entry to parse
   ...
   * <p>
   * The texture filter is specified by a string of lowercase letters. Compound words are separated
   * by spaces.  We use the OpenGL order for filter names, so "linear mipmap nearest" is
   * MipMapLinearNearest.
   *
   * @param json          The JSON entry to parse
