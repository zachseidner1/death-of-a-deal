 ...
 * <p>
 * This class is heavily based on asset manager.  The primary differences are that (1) it is a
 * singleton (so that it can be easily accessed anywhere in the game, and (2) the resources are not
 * loaded from a file.  Effectively, this is a singleton hashtable that allows us to have
 * thread-safe global variables in our application.
 * <p>
 * The primary use of this class is to store parts of our graphics pipeline (shaders, vertex
 * buffers, sprite batches) that will be needed everywhere.  They can be generated in a loading or
 * splash screen and then used globally.  However, it is also useful for storing other features as
 * well.  If you have only one AssetManager, you can even store that in this class.
 * <p>
 * resources stored in this resource manager are heavyweight in the sense that they must be actively
 * disposed (we cannot just rely on the garbage collector).  Such classes must implement
 * {@link Disposable}. When an resource is added to the manager, the manager becomes the resource
 * owner of that resource, and will be responsible for disposing of it. All references to the object
 * outside of the reference manager will be essentially weak references (in the C++ sense).
 */
public class ResourceManager {
   ...
   * <p>
   * There is only one resource manager, so the constructor is not public.  Use this method to
   * access it.
   *
   ...
   * <p>
   * All resources stored in this pipeline manager will be disposed and no longer safe to use. This
   * method should only be called near the end of the application. To prevent a resource from being
   * disposed, it should be removed first.
   */
  ...
  public synchronized <T> String getKey(T resource) {
    for (Class rsrcType : resources.keys()) {
      ...
      for (String key : rsrcByType.keys()) {
        ...
        if (other == resource || resource.equals(other)) {
          return key;
        }
      }
    }
   ...
   * <p>
   * Since the manager owns the resource, it is no longer safe to use it once this method is
   * called.
   *
   * @param key The resource key
   ...
   * <p>
   * Since the manager owns its resources, it is no longer safe to use any of them once this method
   * is called.  This method should only be called near the end of the application.
   */
   ...
   * <p>
   * Calling this method results in a transfer of ownership. The caller now owns the resource and is
   * responsible for disposing it.  The resource manager will no longer dispose of it, even when
   * {@link #clear} is called. That is why this method returns the resource.
   *
   * @param key The resource key
   * @return the resource with the given key
   * @throws GdxRuntimeException if no asset of type T has that key
   ...
   * <p>
   * When assigning a key to a resource, keys should be globally unique, even across types.
   * <p>
   * Calling this method results in a transfer of ownership. The caller no longer owns the resource,
   * having transfered it to the manager. When the manager is deleted, it will dispose of this
   * resource.
   *
   * @param key      The resource key
   * @param type     The resource type
