 ...
 * <p>
 * A film strip asset should be specified by filename:name where name is a unique name for the
 * region.  Note that, unlike textures, film strips are not really reusable. They all share a frame
 * attribute. So each film strip asset must be explicitly copied if you want to make another.
 */
public class FilmStripLoader extends
    AsynchronousAssetLoader<FilmStrip, FilmStripLoader.FilmStripParameters> {
   ...
   */
  @Override
  public FilmStrip loadSync(AssetManager manager, String fileName, FileHandle file,
      FilmStripParameters params) {
  ...
  public FileHandle resolve(String fileName) {
    ...
    if (suffix == -1) {
      throw new GdxRuntimeException(
          "Texture region file name must end in " + FilmStripParser.ALIAS_SEP + "name'.");
    }
   ...
   * <p>
   * A filmstrip is a texture broken up into equal sized components, read top-to-bottom and
   * left-to-right.  It is defined by the number of rows, the number of columns, and the size. The
   * file name is just a symbolic reference.
   */
