 ...
 * <p>
 * All music buffers are named symbolically, since that may span multiple audio sources. They are
 * fully defined by their loader parameters.
 */
public class MusicQueueLoader extends
    AsynchronousAssetLoader<MusicQueue, MusicQueueLoader.MusicQueueParameters> {
  ...
  public void loadAsync(AssetManager manager, String fileName, FileHandle file,
    ...
    if (params.sources.size == 0) {
      ...
      cachedBuffer = ((AudioEngine) Gdx.audio).newMusicBuffer(first.getChannels() == 1,
          first.getSampleRate());
    }
   ...
   */
  @Override
  public MusicQueue loadSync(AssetManager manager, String fileName, FileHandle file,
      MusicQueueParameters params) {
   ...
   * <p>
   * While a music buffer may be derived from an audio source, it can be empty so long as the number
   * of channels (mono/stereo) and sample rate is defined.  Set sampleRate to -1 to use the sample
   * rate of the first source.
   */
  public static class MusicQueueParameters extends AssetLoaderParameters<MusicQueue> {
     ...
     * <p>
     * If sources is not empty, this value will be ignored and the buffer will use the setting of
