 ...
 * <p>
 * As a subinterface of {@link Sound}, this supports pausing, resuming and so on, even on multiple
 * simultaneous instances.  It also adds support for a callback function (see
 * {@link OnCompletionListener}) when an instance has stopped playing.
 * <p>
 * The audio data being play is fully loaded into memory, so this interface is primarily for short
 * clips. While there is no upper limit on the audio file size, you should avoid using this method
 * for any sound asset greater than 1 MB.
 * <p>
 * Sound buffer instances are created via {@link AudioEngine#newSound}. When you are done with using
 * the sound buffer instance you have to dispose it via the {@link #dispose()} method.
 * <p>
...
public interface SoundEffect extends Sound {
   ...
   * <p>
   * LibGDX only supports mono and stereo audio assets. It does not support complex multi-channel
   * assets (such as 7.1 surround).
   *
   ...
   * <p>
   * The sound id should be one given by {@link #play()} or {@link #play(float)}. If the sound id is
   * not valid, this method returns false.
   *
   ...
   * <p>
   * The sound id should be one given by {@link #play()} or {@link #play(float)}. If the sound id is
   * not valid, this method returns -1.
   *
   * @param soundId The playback instance
   ...
   * <p>
   * The sound id should be one given by {@link #play()} or {@link #play(float)}. If the sound id is
   * not valid, this method returns 0.
   *
   * @param soundId The playback instance
   ...
   * <p>
   * The sound id should be one given by {@link #play()} or {@link #play(float)}. If the sound id is
   * not valid, this method has no effect.  A value outside of the bounds of the assets sets the
   ...
   * <p>
   * This callback is essentially the same as{@link Music.OnCompletionListener}. It only differs in
   * the fact that the sound object may have multiple instances playing simultaneously, so it must
   * specify the particular instance that has just ended.
   */
